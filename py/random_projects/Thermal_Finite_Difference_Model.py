...
def buildwall(nodesperlayer, w, layers, layer, layertime, cooldowntime, alpha, c, rho, k, Tdep, Tinf, Tb, h, emissivity, counter1, counter2, u):
    ...
    timedataplot = Tnodes[0:(timepoints*layers+cooldowntimepoints+1)                          :timeskip, 1:nodesperlayer*layers:nodesperlayer]
...
# time_data_plot = np.empty((0, 1), np.float)
...
for i in range(lenw):
    ...
    for j in range(lenlayertime):
        ...
        layertime_temp = layertime[j] if isinstance(
            layertime, list) or isinstance(layertime, tuple) else layertime
