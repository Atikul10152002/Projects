...
function [Tss,delx,delt,timepoints,cooldowntimepoints,spatialdataplot,timedataplot,timeskip]= buildwall(nodesperlayer,w,layers,layer,layertime,cooldowntime,alpha,c,rho,k,Tdep,Tinf,Tb,h,emissivity,counter1,counter2,u)
...
// (*Thermal details*)
 ...
 nodes = [1:nodesperlayer];
 
 deltmaxbot = (12*k/(rho*c*delx^2) + 2*(h+hradmax)/(rho*c*w))^(-1);
 deltmaxtop = (2*k/(rho*c*delx^2) + 2*(h + hradmax)/(rho*c*w) + 2*(h + hradmax)/(rho*c*delx))^(-1);
 ...
 timepoints = floor(layertime/delt) + 1;
 delt = layertime/timepoints;
 cooldowntimepoints = floor(cooldowntime/delt);
 temporary = 0;
...
Tnodes = zeros(timepoints*layers+cooldowntimepoints, nodesperlayer*layers);
Qtop = zeros(timepoints*layers+cooldowntimepoints, 1);
Qwall = zeros(timepoints*layers+cooldowntimepoints, 1);
Qbed = zeros(timepoints*layers+cooldowntimepoints, 1);

 // Build the wall
 for n = 1:layers
   if n==1 then
       Tnodes(1, [1:nodesperlayer]) = Tdep;
   elseif n==layers
       temporary = cooldowntimepoints;
       Tnodes(((n-1)*timepoints + 1), [(n-1)*nodesperlayer+1:n*nodesperlayer]) = Tdep;
   else
       Tnodes(((n-1)*timepoints + 1), [(n-1)*nodesperlayer+1:n*nodesperlayer]) = Tdep;
   end
...
//    (*Bottom node with constant temperature BC*)
    hrad = emissivity*sigma*(Tnodes(j, 1) + Tinf)*(Tnodes(j, 1)^2 + Tinf^2);
...
//    (*Middle nodes with convective cooling and conduction*)
    ...
    for i = 2:n*nodesperlayer-1
     hrad = emissivity*sigma*(Tnodes(j, i) + Tinf)*(Tnodes(j, i)^2 + Tinf^2);
